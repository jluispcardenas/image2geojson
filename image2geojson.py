...
def convert(file, output, lng, lat):
    ...
    ret, th1 = cv2.threshold(gray, 50, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)

    contours, hierarchy = cv2.findContours(th1, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    geojson = {"type": "FeatureCollection", "creator" : "image2geojson", "features": []}
    for cont in contours:
        if len(cont) < 3:
            continue

        item = {"type": "Feature", "properties": None, "geometry": {"type": "Polygon", "coordinates": [[]]}}
        coordinates = []
        for p in cont:
...
def convert_to_lonlat(x, y, lng, lat):
    ...
    lng_diff = (x / r_earth) * (180 / pi)
    lat_diff = (y / r_earth) * (180 / pi) / math.cos(lat * pi/180)
